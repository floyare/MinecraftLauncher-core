...
class MCLCore extends EventEmitter {
  ...
  async getModifyJson () {
    ...
    if (this.options.forge) {
      ...
      modifyJson = modifyJson || JSON.parse(fs.readFileSync(path.join(this.options.overrides.versionJson), { encoding: 'utf8' })) || JSON.parse(fs.readFileSync(path.join(this.options.root, 'versions', this.options.version.custom, `${this.options.version.custom}.json`), { encoding: 'utf8' }))
    }
